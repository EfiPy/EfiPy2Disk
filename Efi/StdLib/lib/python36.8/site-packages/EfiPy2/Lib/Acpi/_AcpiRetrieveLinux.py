...
def FindRsdpBaseLinux ():
  ...
  try:
    import os
    SysTableName = '/sys/firmware/efi/systab'
    SysTableSize = os.path.getsize (SysTableName)
    SysTableHandle = open (SysTableName, 'rb')
    ...
    return Acpi20Offset, SysTableSize   # Base, Size
...
def LoadAcpiTableLinux (MmapFile, TableBase, TableSize, Signature = None):
  ...
  if Signature != None:
    TableOffset = MmapHandle[Offset: TableSize - Offset].find (Signature) + Offset
  else:
    TableOffset = Offset
